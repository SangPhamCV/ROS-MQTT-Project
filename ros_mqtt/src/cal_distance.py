...
topic_mqtt = rospy.get_param('mqtt_pub_estimate', "robot/estimate")

def callback(path_msg):
    global path_length
    path_length = 0

    for i in range(len(path_msg.poses) - 1):        # calcutate distance fomular
    ...
    # when the robot's pose has changed, clear the path_distance and recalculated
